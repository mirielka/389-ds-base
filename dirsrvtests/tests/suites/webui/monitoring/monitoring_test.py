...
def test_replication_visibility(topology_st, page, browser_name):
    ...
    #frame.get_by_role('treeitem', name='replication-suffix-dc\\=example\\,dc\\=com').click()
    frame.locator('#replication-suffix-dc\\=example\\,dc\\=com').click()

    frame.locator('#dc\\=example\\,dc\\=com-agmts').click()
    assert frame.get_by_text('Replication Agreements')

    frame.locator('#dc\\=example\\,dc\\=com-winsync').click()
    assert frame.get_by_text('Winsync Agreements')
