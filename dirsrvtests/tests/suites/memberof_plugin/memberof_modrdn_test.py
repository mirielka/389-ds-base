...
from dirsrvtests.tests.suites.memberof_plugin.memberof_skipnested_test import (
    user1, group1, check_membership)
import pytest
import os
import logging
...
def test_memberof_modrdn(topology_st, user1, group1):
    ...
    found = False
    for i in topology_st.standalone.ds_error_log.match('.*Skip modrdn operation because src/dst identical.*'):
